...
convert_HiCPro_OUT_to_WUbrowser_IN = function(hicp_matrix_file, hicp_bed_file, cell, binSize, chr, out_dir = "~/R/HiC_histone_genes/wubrowser_format/"){
  ...
  topL = chr_matrix_dt[, .(seqnames = r1_seqnames, start = r1_start, end = r1_end, pair = paste0(r2_seqnames, ":", r2_start, "-", r2_end, ",", val))]
  topL$index = 1:nrow(topL)
  topL$strand = "."
  botL = chr_matrix_dt[, .(seqnames = r2_seqnames, start = r2_start, end = r2_end, pair = paste0(r1_seqnames, ":", r1_start, "-", r1_end, ",", val))]
  botL$index = 1:nrow(botL)
  botL$strand = "."
  joined = rbind(topL, botL)
  joined = joined[order(index)]
  joined$index = 1:nrow(joined)
