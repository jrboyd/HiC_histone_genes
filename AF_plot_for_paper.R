...
simp_ref = simplify_ref(simp_ref, "^ZKS")
...
# regions = read.table("~/HiC-Pro/outputs/MCF10A_pooled/hic_results/matrix/MCF10A_20split/raw/40000/MCF10A_20split_40000_abs.bed")
# colnames(regions) = c("seqnames", "start", "end", "id")
# regions_gr = GRanges(regions)
# regions_gr$rownames = ""
# for(l in levels(seqnames(regions_gr))){
#   k = as.logical(l == seqnames(regions_gr))
#   regions_gr$rownames[k] = paste(l, regions_gr$id - min(regions_gr$id) + 1)
# }
# to_keep = which(as.logical(seqnames(regions_gr) == "chr6"))
# rn = regions_gr$rownames[to_keep]
# files = system(command = "ls ~/HiC-Pro/outputs/*ed/hic_results/matrix/*/iced/40000/*matrix", intern = T)
# all_dt = lapply(files, function(f){
#  x = fread(f) 
# })

# 
# all_binned = lapply(all_dt, function(x){
#   x = x[V1 %in% to_keep & V2 %in% to_keep]
#   x[, c("V1", "V2") := .(V1 - min(to_keep) , V2 - min(to_keep) )]
#   diag = x[V1 == V2]
#   main = x[V1 != V2]
...
#           j = x$V2, 
#           x=x$V3, 
#           Dim= c(length(to_keep), length(to_keep)))
#   # m = as.matrix(M)
#   rownames(M) = rn
#   colnames(M) = rn
#   M
# })
# bin_size = 40000
# names(all_binned) = c("MCF10A", "MCF10AT1", "MCF10CA1")
...
my_plots = function(binned_name, loci_gr, gene_ref, ext, quantile_cap = 1){
