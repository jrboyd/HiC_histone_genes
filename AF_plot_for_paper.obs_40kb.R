...
# j_files = dir("juicer_output/40kb", pattern = "obs_vs", full.names = T)
...
outdir = "output_AF_paper_obs_40kb"
...
for(i in (1:length(all_binned))){
  for(j in 1:length(my_loci)){
    ...
    pdf(paste0(root, "_obs_40kb_scale_quantile.pdf"), width = 6, height = 8)
